...
void sensors_get_device_id(char *id_buffer, size_t buffer_size)
{
    ...
    snprintf(id_buffer, buffer_size, "%02X%02X%02X%02X%02X%02X", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
}
...
void sensors_init(void)
{
    ...
    ESP_LOGI(TAG, "Датчики уровня инициализированы (GPIO %d, %d)", LEVEL_SENSOR_1_GPIO, LEVEL_SENSOR_2_GPIO);
